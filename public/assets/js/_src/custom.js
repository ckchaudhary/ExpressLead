jQuery(function () {
	...
	setTimeout( function(){
		// First, Right after page load.
		$(".js-animate-into-viewport:not(.animated-into-viewport)").each(function() {
			let $this = $(this);
			if ( isTopVisible( $this ) ) {
				$this.addClass( 'animated-into-viewport' );
				slideanimate( $this );
				startCounter( $this );
			}
		});
	}, 100 );// A small delay is required so that the slider is setup by then. Otherwise the offset of all elements below the slider is messed up.
	...
	let $bannerSlider = $('.hero-section-slider');
	let $bannerFirstSlide = $('.hero-section-slider .hero-slider-img:first-child');
	...
	jQuery('.js-st-cf').on( 'click', function(e){
		e.preventDefault();
		...
		$([document.documentElement, document.body]).animate({
			...
			setTimeout( function(){
				$target.removeClass( 'animate__animated animate__headShake' );
			}, 3000 );
		}) ;
	});

	// Populate countries options
	if ( jQuery('.js-countries-select').length > 0 ) {
		let countries_options = '';
		let countries_list = getCountriesList();
		let continents = [ 'Africa', 'Antarctica', 'Asia', 'Europe', 'North America', 'Oceania', 'South America' ];
		for ( let this_continent of continents ) {
			countries_options += '<optgroup label="' + quoteattr( this_continent ) + '">';
			for ( let ccode in countries_list ) {
				let country = countries_list[ ccode ];
				if ( country.continent !== this_continent ) {
					continue;
				}

				let country_name = country.name;
				country_name.stripHTML();
				country_name.stripSlashes();
				countries_options += '<option value="'+ quoteattr( ccode ) +'" data-isd="'+ quoteattr( country.isd ) +'">'+ country_name +'</option>';
			}
			countries_options += '</optgroup>';
		}

		jQuery('.js-countries-select').append( countries_options );
	}

	jQuery('#companyCountry').change(function(){
		let newval = $(this).find('option:selected').attr('data-isd');
		if ( newval ) {
			$('#phone').closest('.input-group').find('.input-group-text').text('+' + newval);
		}
	});
	jQuery('.js-countries-select').select2({
		dropdownCssClass: 'style1',
	});

	// Form submit event.
	$('form.get-in-touch-form').submit(function(e){
		e.preventDefault();
		const $form = $(this);
		if ( $form.data('doing-ajax') ) {
			return false;
		}

		$form.data('doing-ajax', true);
		$.ajax({
			...
			let message = Object.prototype.hasOwnProperty.call(data, 'message') ? data.message : 'Connection lost. Request couldn\'t be completed. Please try again.';
			message = '<p class="alert alert-danger">' + message + '</p>';

			// Show error message
			let modal = new LogiPrismModal( message );
			modal.show();
		}).done(function( res ){
			let data = JSON.parse( res );
			let message = Object.hasOwn(data, 'message') ? data.message : '';
			if ( message ) {
				message = data.success ? '<p class="alert alert-success">' + message + '</p>' : '<p class="alert alert-danger">' + message + '</p>';
				new LogiPrismModal( message ).show();
			}
			$form.get(0).reset();
		}).always(function(){
			$form.data('doing-ajax', false);
		});
	});

	maybeShowGlobalMessages();

	// tabs
	$(document).on("click", ".offerings-section-row .menu div", function() {
		var numberIndex = $(this).index();

		if (!$(this).is("active")) {
			$(".offerings-section-row .menu div").removeClass("active");
			$(".offerings-section-row ul li").removeClass("active");

			$(this).addClass("active");
			$(".offerings-section-row ul").find("li:eq(" + numberIndex + ")").addClass("active");

			var listItemHeight = $(".offerings-section-row ul")
				.find("li:eq(" + numberIndex + ")")
				.innerHeight();
			$(".offerings-section-row ul").height(listItemHeight + "px");
		}
	});
...
function maybeShowGlobalMessages () {
	let all_messages = [];

	// First check cookies
	if ( typeof ( 'docCookies' ) !== 'undefined' ) {
		let cmessage = docCookies.getItem( 'lp-messages' );
		if ( cmessage != '' && cmessage !== null ) {
			all_messages = jQuery.parseJSON( cmessage );
			// delete cookies
			docCookies.removeItem( 'lp-messages', '/' );
		}
	}

	// Then footer script
	if ( typeof( window.iazo_messages ) !== 'undefined' ) {
		for ( const gm of window.iazo_messages ) {
			let is_already_added = false;
			if ( all_messages.length ) {
				for ( const cm of all_messages ) {
					if ( cm['i'] == gm['i'] ) {
			...
			if ( !is_already_added ) {
				all_messages.push( gm );
			}
		}
	}

	// Now show'em one by one
	if ( all_messages.length ) {
		for ( const am of all_messages ) {
			let message_content = am.m.replace(/\+/g,' ');
			let modal = new LogiPrismModal( message_content );
			modal.modal_class = am.t;
			modal.show();
		}
	}
};

class LogiPrismModal {
    fetch_from = '';
    target_id = '';
    modal_class = '';
	content = '';
	...
	<div class="lpmodal">
		...
		<div class="lightbox-wrapper">
			<div class="lightbox-inner MODAL_CLASS">
    ...
    constructor( content = '' ) {
        ...
        // Bind Events
        jQuery('body').on('click', '.lpmodal .modal-close', function(e) {
            e.preventDefault();
            _this.hide();
        });
    };
    ...
    show() {
        const _this = this;
        jQuery(".lpmodal").remove();
        let template = _this.template.replace('MODAL_CLASS', _this.modal_class);
        jQuery("body").addClass("has-lpmodal").append(template);

        if (_this.fetch_from) {
            jQuery.ajax({
                method: 'GET',
                url: _this.fetch_from,
            }).done(function(response) {
                jQuery(".lpmodal .lightbox-inner").html('<div class="close modal-close"></div>' + response);
            });
        }
        if (_this.target_id) {
            let html = jQuery('#' + _this.target_id).html();
            jQuery(".lpmodal .lightbox-inner").html('<div class="close modal-close"></div>' + html);
        }
		if ( _this.content != '' ) {
			jQuery(".lpmodal .lightbox-inner").html('<div class="close modal-close"></div>' + _this.content );
		}
    };

    hide() {
        jQuery(".lpmodal").remove();
        jQuery("body").removeClass("has-lpmodal");
    };
}
...
String.prototype.stripHTML = function(){
    return jQuery('<p/>').html(this.toString()).text();
}
